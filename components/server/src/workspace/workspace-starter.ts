...
export class WorkspaceStarter {
    ...
    public async startWorkspace(ctx: TraceContext, workspace: Workspace, user: User, userEnvVars?: UserEnvVar[], rethrow?: boolean, forceDefault: boolean = false): Promise<StartWorkspaceResult> {
        ...
        log.info(`The resolved image name is ${JSON.stringify(workspace.imageSource)}`)
        console.log(`The resolved image name is ${JSON.stringify(workspace.imageSource)}`)
        log.debug(`phase 3 value: ${forceDefault}`)
        ...
        try {
            ...
            if (needsImageBuild && !rethrow) {
                this.actuallyStartWorkspace({ span }, instance, workspace, user, userEnvVars, rethrow);
    ...
    protected async newInstance(workspace: Workspace, user: User): Promise<WorkspaceInstance> {
        ...
        // privileged is a special case which requires the privileged-ws permission
        if (featureFlags.includes("privileged") && !this.authService.hasPermission(user, "privileged-ws")) {
